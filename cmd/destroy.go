...
func runDestroyCmd(cmd *cobra.Command, args []string) {
	...
	expandedBlueprintFile := filepath.Join(getArtifactsDir(deplRoot), modulewriter.ExpandedBlueprintName)
	dc, _, err := config.NewDeploymentConfig(expandedBlueprintFile)
	checkErr(err)

	checkErr(shell.ValidateDeploymentDirectory(dc.Config.DeploymentGroups, deplRoot))
