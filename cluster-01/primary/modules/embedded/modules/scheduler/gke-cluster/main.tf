    ...
    for idx, network_info in var.additional_networks : [
      {
        ...
        template_vars = {
          # TODO: revisit the prefix
          name            = "${network_info.network}-${network_info.subnetwork}",
      ...
      },
      {
        ...
        # TODO: revisit the prefix
        template_vars = { name = "${network_info.network}-${network_info.subnetwork}" }
      }
    ]
  ])
}
...
resource "google_container_cluster" "gke_cluster" {
  ...
  project         = var.project_id
  name            = local.name
  # TODO(arajmane): Revert
  location        = "us-east5-staginga"
...
#         template_vars = {
#           # TODO: revisit the prefix
#           name            = "${network_info.network}-${idx + 1}",
#           network_name    = network_info.network
#           subnetwork_name = network_info.subnetwork
#           device_mode     = strcontains(upper(network_info.nic_type), "RDMA") ? "RDMA" : "NetDevice"
#         }
