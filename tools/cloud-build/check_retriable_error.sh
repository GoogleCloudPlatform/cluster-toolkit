...
RETRIABLE_ERRORS="ZONE_RESOURCE_POOL_EXHAUSTED|does not have enough resources available|not enough resources available|resource exhausted|stockout|was created in the error state.*ERROR.*|srun: error: Node failure on|srun: error: Nodes .* are still not ready|os-login.*ssh-keys.*add|resourceInUseByAnotherResource|Error acquiring the state lock|412 Precondition Failed|conditionNotMet|RATE_LIMIT_EXCEEDED|Mutate requests per minute|Error 429|HTTP 429|429 Too Many Requests|Error 50[0-9]|Internal error|backendError|Service Unavailable|connection reset by peer|TLS handshake timeout|overlaps with the existing allocated IP range|Connection refused|Connection timed out|Failed to connect to the host via ssh|Failed to create cluster|An internal exception occurred|serviceAccount was not found"
