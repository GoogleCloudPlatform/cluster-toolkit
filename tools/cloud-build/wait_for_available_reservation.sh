...
if [[ -z "${RESERVATION}" || -z "${ZONE}" ]]; then
	echo "Usage: $0 <RESERVATION_NAME> <ZONE> [PROJECT_ID]"
	exit 1
fi
...
while true; do
	...
	(
		if ! OUTPUT=$(gcloud compute reservations describe "${RESERVATION}" \
			--zone="${ZONE}" \
			"${PROJECT_ARGS[@]}" \
			--format="value(specificReservation.count, specificReservation.inUseCount)" 2>&1); then
		...
		# Ensure we received valid numeric data
		...
		if [[ ${AVAILABLE} -gt 0 ]]; then
			...
			# Capacity is full, echo specific error string to be grepped below
