...
def parse_args(argv=None):
    ...
    smoke_client.add_client_arguments(parser, include_api_format=False)
    ...
    args.api_format = "anthropic" if args.provider == "anthropic" else "openai"
    ...
    if args.api_format == "anthropic":
        args.client_arguments += ["--api-format", "anthropic"]
    return args
...
def main():
    ...
    pod = next((p for p in pods if (p.get("status") or {}).get("phase") == "Running"), None)
    if pod is None:
        raise ValueError("No running agentgateway-proxy pod found")
