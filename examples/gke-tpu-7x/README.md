...
This feature automatically configures the GKE cluster (Version: 1.35.0-gke.3065000 or higher) with the necessary settings, labels the user namespace, and configures the Workload Identity service accounts required by the ML Diagnostics SDK.
...
3. **Configure GKE-CLUSTER:** Ensure the `gke-tpu-7x-cluster` module is configured to enable ML diagnostics and create the Workload Identity resources in the user namespace.
...
To test with sample workload, refer to the [ML Diagnostics Test README](../gke-tpu-v6e/ml-diagnostics-sample-workload-test/README.md). This guide explains how to build a test image and run a job to verify metrics and profiling in the Google Cloud Console.
