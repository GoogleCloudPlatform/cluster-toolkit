...
```bash
srun -N 4 -p compute -w highioslur-compute-c2s60-[0-3] hostname
```
...
Partition: lowcost; Node Group: n2s2; Machine Type: n2-standard-4

```bash
srun -N 4 -w highioslur-lowcost-n2s4-[0-3] hostname
```
...
* `compute`
  * Includes two node groups, `c2s60` of machine type `c2-standard-60` and
    `c2d112` of machine type `c2d-standard-112`.
...
* Compute Engine API: C2 CPUs:
  ...
  * Maximum possible: **12,008**
* Compute Engine API: C2D CPUs: **112/node**, maximum **22,400**
* Compute Engine API: Affinity Groups: **one for each job in parallel** - _only
  needed for `compute` partition_
* Compute Engine API: Resource policies: **one for each job in parallel** -
  _only needed for `compute` partition_
