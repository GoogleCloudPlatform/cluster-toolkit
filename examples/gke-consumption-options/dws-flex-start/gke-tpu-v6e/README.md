...
1. In the `examples/gke-consumption-options/dws-flex-start/gke-tpu-v6e/gke-tpu-v6e-deployment.yaml` file, fill in the following settings in the terraform_backend_defaults and vars sections to match the specific values for your deployment:
    ...
    **`autoscaling_max_node_count`**: set to the required node count for your topology (e.g., `4` for a `4x4` topology).
    ...
    To modify advanced settings, edit `examples/gke-consumption-options/dws-flex-start/gke-tpu-v6e/gke-tpu-v6e.yaml`.
...
Each pod in the job should request the full amount of TPU chips available on the node (typically 4 for `ct6e-standard-4t`):
...
The `parallelism` and `completions` count in your Kubernetes Job (or ReplicatedJob replicas in a JobSet) must exactly match the number of nodes in your TPU slice (e.g., `4` for a `4x4` topology).
...
3. **Observe Scale-Up:**
   ...
   * **Initial State:** Pods will show as `Pending`.
    ...
    my-job-775c-0-stfds   0/2     Pending   0          10s
    ```
   ...
   * **Nodes Joining:** After a few minutes, nodes will appear and transition to `Ready`.
    ...
    gke-tpu-46f78452-4hkf   Ready    <none>   10s   v1.33.5-gke.2172000
    ```
...
4. **Observe Scale-Down:**
   ...
   * **Completion:** Once the workload finishes, pods will move to `Completed`.
    ...
    ```text
    NAME                READY   STATUS      RESTARTS   AGE
    my-job-775c-0-stfds   0/2     Completed   0          5m
    ```

   * **Automatic Removal:** After a short idle period (typically 1-10 minutes), the Cluster Autoscaler will delete the nodes.

    ```text
    gke-tpu-46f78452-4hkf   NotReady,SchedulingDisabled   <none>   6m   v1.33.5-gke.2172000
    ```
