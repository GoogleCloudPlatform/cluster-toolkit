...
| <a name="input_maintenance_exclusions"></a> [maintenance\_exclusions](#input\_maintenance\_exclusions) | List of maintenance exclusions. A cluster can have up to three. exclusion\_end\_time\_behavior is the exclusion window end time behavior. One of 'UNTIL\_END\_OF\_SUPPORT' end\_time\_behavior or end\_time should be specified. | <pre>list(object({<br/>    name                        = string<br/>    start_time                  = string<br/>    end_time                    = string<br/>    exclusion_scope             = string<br/>    exclusion_end_time_behavior = optional(string)<br/>  }))</pre> | `[]` | no |
