...
variable "cloud_dns_config" {
  ...
  additive_vpc_scope_dns_domain: This will enable Cloud DNS additive VPC scope.
  Must provide a domain name that is unique within the VPC.
  For this to work cluster_dns = "CLOUD_DNS" and cluster_dns_scope = "CLUSTER_SCOPE" must both be set as well.
  ...
  validation {
    condition     = !(var.cloud_dns_config.additive_vpc_scope_dns_domain != null && var.cloud_dns_config.cluster_dns == "CLOUD_DNS" && var.cloud_dns_config.cluster_dns_scope == "CLUSTER_SCOPE")
