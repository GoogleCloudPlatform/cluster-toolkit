...
handle_debian() {
	...
	add_repo() {
		...
		if ! cmp -s - /etc/apt/sources.list.d/google-cloud-monitoring.list <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/apt/sources.list.d/google-cloud-monitoring.list <<<"${REPO_DATA}"
			CHANGED=1
		...
		run_with_retry ${DRY_RUN} apt_get_update \
			-o Dir::Etc::sourcelist="sources.list.d/google-cloud-monitoring.list" \
...
gpgkey=https://${REPO_HOST}/yum/doc/yum-key.gpg
       https://${REPO_HOST}/yum/doc/rpm-package-key.gpg"
		...
		if ! cmp -s - /etc/yum.repos.d/google-cloud-monitoring.repo <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/yum.repos.d/google-cloud-monitoring.repo <<<"${REPO_DATA}"
		...
		run_with_retry ${DRY_RUN} yum -y \
			--disablerepo="*" --enablerepo=google-cloud-monitoring \
...
gpgkey=https://${REPO_HOST}/yum/doc/yum-key.gpg
       https://${REPO_HOST}/yum/doc/rpm-package-key.gpg"
		...
		if ! cmp -s - /etc/zypp/repos.d/google-cloud-monitoring.repo <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/zypp/repos.d/google-cloud-monitoring.repo <<<"${REPO_DATA}"
			CHANGED=1
		fi
		...
		if [[ -n "${DRY_RUN:-}" ]] || ! cmp --silent - <(rpm --query gpg-pubkey) <<<"${RPM_KEYS}"; then
			CHANGED=1
		fi
