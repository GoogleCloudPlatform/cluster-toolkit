...
handle_debian() {
	...
	add_repo() {
		...
		if ! cmp -s - /etc/apt/sources.list.d/google-cloud-ops-agent.list <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/apt/sources.list.d/google-cloud-ops-agent.list <<<"${REPO_DATA}"
			CHANGED=1
...
gpgkey=${GPG_KEYS}"
		...
		if ! cmp -s - /etc/yum.repos.d/google-cloud-ops-agent.repo <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/yum.repos.d/google-cloud-ops-agent.repo <<<"${REPO_DATA}"
...
gpgkey=https://${REPO_HOST}/yum/doc/yum-key.gpg
       https://${REPO_HOST}/yum/doc/rpm-package-key.gpg"
		...
		if ! cmp -s - /etc/zypp/repos.d/google-cloud-ops-agent.repo <<<"${REPO_DATA}"; then
			echo "Adding agent repository for ${ID}."
			${DRY_RUN} tee /etc/zypp/repos.d/google-cloud-ops-agent.repo <<<"${REPO_DATA}"
			CHANGED=1
		fi
