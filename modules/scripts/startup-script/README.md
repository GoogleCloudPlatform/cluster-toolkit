...
| <a name="input_prepare_slurm_scripts"></a> [prepare\_slurm\_scripts](#input\_prepare\_slurm\_scripts) | Configures Slurm Prolog & Epilog Scripts across different machine types | <pre>object({<br/>    accelerator_profile = optional(string, "")<br/>    common_scripts      = optional(list(string), [])<br/>  })</pre> | <pre>{<br/>  "accelerator_profile": "",<br/>  "common_scripts": []<br/>}</pre> | no |
