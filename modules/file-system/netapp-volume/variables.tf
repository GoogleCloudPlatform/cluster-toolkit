...
variable "dns_config" {
  ...
  validation {
    condition     = var.dns_config == null || !strcontains(coalesce(var.dns_config.record_name, var.volume_name), "_")
    error_message = "The DNS record_name (or volume_name if record_name is not provided) cannot contain underscores. DNS hostnames allow only letters, digits, and hyphens. Use hyphens (-) instead."
  }
}
