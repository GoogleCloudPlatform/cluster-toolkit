...
| <a name="input_create_gcsfuse_iam_role"></a> [create\_gcsfuse\_iam\_role](#input\_create\_gcsfuse\_iam\_role) | Whether to automate the creation of the custom IAM role and GKE Service Agent binding for GCS Fuse Anywhere Cache. Set to false in restricted environments where the deployer lacks project-level IAM permissions. | `bool` | `true` | no |
