...
locals {
  ...
  pvc_template_vars = {
    gcs       = local.common_pvc_vars
    lustre    = local.common_pvc_vars
    filestore = local.common_pvc_vars
  }

  # Variables for PV templates (specific to each storage type)
  pv_template_vars = {
    gcs = {
      pv_name       = local.pv_name
      capacity      = "${var.capacity_gb}Gi"
  ...
  pvc_content = templatefile(
    ...
    local.pvc_template_vars[local.storage_type]
  )
