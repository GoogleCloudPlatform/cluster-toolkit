...
| <a name="input_specific_reservation"></a> [specific\_reservation](#input\_specific\_reservation) | Reservation resources to consume when targeting SPECIFIC\_RESERVATION. Specify `compute.googleapis.com/reservation-name` as the key and the list of reservation names as the value. | <pre>object({<br>    key    = string<br>    values = list(string)<br>  })</pre> | <pre>{<br>  "key": null,<br>  "values": null<br>}</pre> | no |
