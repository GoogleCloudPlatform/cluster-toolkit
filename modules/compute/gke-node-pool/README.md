...
| <a name="input_enable_dranet"></a> [enable\_dranet](#input\_enable\_dranet) | Enable GKE managed Dynamic Resource Allocation (DRA) driver for networking (DRANET) and Accelerator Network Profile (ANP). CRITICAL: This MUST be set to true for multi-slice/multi-network TPU or GPU clusters to automatically configure host policy routing and prevent distributed collectives (such as JAX or NCCL) from hanging due to hypervisor-level asymmetric routing packet drops. | `bool` | `false` | no |
