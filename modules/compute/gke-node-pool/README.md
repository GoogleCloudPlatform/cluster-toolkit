...
| <a name="input_install_dranet_template"></a> [install\_dranet\_template](#input\_install\_dranet\_template) | If true, automatically deploys the DRANET ResourceClaimTemplate. Set to false if another node pool in the same cluster is already deploying it for the same device class. | `bool` | `true` | no |
