...
| <a name="input_claims"></a> [claims](#input\_claims) | A list of claims in the container resources block (e.g. [{ name = "dranet-network" }]). | <pre>list(object({<br/>    name = string<br/>  }))</pre> | `[]` | no |
...
| <a name="input_dranet_template_name"></a> [dranet\_template\_name](#input\_dranet\_template\_name) | The name of the DRANET ResourceClaimTemplate to use. Automatically inherited from the node pool if 'use' is set. | `list(string)` | `[]` | no |
| <a name="input_enable_dranet"></a> [enable\_dranet](#input\_enable\_dranet) | Boolean indicating whether DRANET is enabled on the target node pool. | `list(bool)` | <pre>[<br/>  false<br/>]</pre> | no |
...
| <a name="input_resource_claims"></a> [resource\_claims](#input\_resource\_claims) | A list of resource claims at the Pod level, defining name and resource\_claim\_template\_name. | <pre>list(object({<br/>    name                         = string<br/>    resource_claim_template_name = string<br/>  }))</pre> | `[]` | no |
