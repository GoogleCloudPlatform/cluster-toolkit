...
locals {
  ...
  provisioning_model = var.preemptible ? "SPOT" : null
  ...
  automatic_restart                  = local.compact_placement || var.preemptible ? false : null
  on_host_maintenance_from_placement = local.compact_placement || var.preemptible ? "TERMINATE" : "MIGRATE"
