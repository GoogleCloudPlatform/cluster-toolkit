...
|------|-------------|------|---------|:--------:|
| <a name="input_always_include_deployment_name"></a> [always\_include\_deployment\_name](#input\_always\_include\_deployment\_name) | If true, the deployment name will always be included as part of the VM name. <br>If false, it will only be included if `name_prefix` is not provided. | `bool` | `false` | no |
