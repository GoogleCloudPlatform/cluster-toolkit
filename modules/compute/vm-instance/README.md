...
| <a name="input_use_deployment_name_in_resource_name"></a> [use\_deployment\_name\_in\_resource\_name](#input\_use\_deployment\_name\_in\_resource\_name) | If true, the deployment name will be included as part of the VM name. <br>If false, it will never be included.<br>Default of null will use deployment name only if `name_prefix` is not provided. | `bool` | `null` | no |
