...
| <a name="input_network_bgp_always_compare_med"></a> [network\_bgp\_always\_compare\_med](#input\_network\_bgp\_always\_compare\_med) | If set to true, the Cloud Router will use MED values from the peer even if the AS paths differ. Default is false. | `bool` | `false` | no |
| <a name="input_network_bgp_best_path_selection_mode"></a> [network\_bgp\_best\_path\_selection\_mode](#input\_network\_bgp\_best\_path\_selection\_mode) | Specifies the BGP best path selection mode. Valid values are STANDARD or LEGACY. Default is LEGACY. | `string` | `"LEGACY"` | no |
