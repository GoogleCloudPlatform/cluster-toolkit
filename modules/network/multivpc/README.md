...
| <a name="input_ips_per_nat"></a> [ips\_per\_nat](#input\_ips\_per\_nat) | The number of static IP addresses to allocate for each regional Cloud NAT when nat\_ip\_allocate\_option is MANUAL\_ONLY (set to 0 to disable NAT unless nat\_ip\_allocate\_option is AUTO\_ONLY) | `number` | `2` | no |
...
| <a name="input_min_ports_per_vm"></a> [min\_ports\_per\_vm](#input\_min\_ports\_per\_vm) | Minimum number of ports allocated to a VM from Cloud NAT. When enable\_dynamic\_port\_allocation is true, must be a power of 2 between 32 and 32768. | `number` | `32` | no |
