locals {
  ...
  # Regex to capture strictly major.minor.patch integers, ignoring everything after
  ...
  # Map to integers, defaulting to 0 for missing patch versions
  ...
  ver_patch = local.is_valid_semver ? (local.parsed_ver[2] != "" ? tonumber(local.parsed_ver[2]) : 0) : 0
  ...
  min_patch = local.is_valid_semver ? (local.parsed_min[2] != "" ? tonumber(local.parsed_min[2]) : 0) : 0
