...
Run a temporary curl Pod to send a request through the internal load balancer.
Your local machine needs [Kubernetes API access](#before-you-deploy) and permission
to create and attach to Pods; it does not need a direct VPC connection. Use your
configured namespace if it differs from `agentgateway-system`.
...
Send a synchronous Gemini request:

```sh
kubectl -n agentgateway-system run agw-curl \
...
The response contains an OpenAI-compatible chat completion. The configured
`vertex_model` takes precedence over the request model. This example uses the
default `gemini-2.5-flash`; omit `thinking_config` when using a model that does not
support it.

To receive streaming events, rerun the command with `"stream": true` and add
`--no-buffer` to curl's options. The `--rm` flag removes the Pod when the command
finishes. Both the internal load balancer and proxy routes use 300-second request
timeouts.

Curl provides a manual connectivity and inference check. For automated response,
stream-completion, private-exposure, and metrics checks, use the
[Kubernetes runner](#run-the-smoke-test-in-a-kubernetes-job).

Alternatively, from a client with network access to the gateway's internal IP,
run the Python smoke client directly to validate synchronous and streaming
responses:
...
VPC, use the gateway IP obtained [above](#send-a-request-with-curl):
