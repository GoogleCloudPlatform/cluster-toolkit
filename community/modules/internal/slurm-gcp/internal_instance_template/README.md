...
| <a name="input_resource_policy_self_link"></a> [resource\_policy\_self\_link](#input\_resource\_policy\_self\_link) | Optional self\_link of a Google Compute Resource Policy to attach to this instance template. Used for features like compact placement. If provided, 'on\_host\_maintenance' will be set to 'TERMINATE'. | `string` | `null` | no |
