...
locals {
  ...
  known_project_families = {
    schedmd-slurm-public = [
      ...
      "slurm-gcp-6-9-ubuntu-2204-lts-arm64"
    ]
  }

  project = var.instance_image.project
  family  = can(var.instance_image.family) ? var.instance_image.family : ""
  name    = can(var.instance_image.name) ? var.instance_image.name : ""
  ...
  project_normalized = (can(var.instance_image.family) ?
    "projects/${local.project}/global/images/family" :
    "projects/${local.project}/global/images"
  )

}

output "source_image_project_normalized" {
  value       = local.project_normalized
  description = "Normalized project id"

  precondition {
    condition     = length(regexall("^projects/.+?/global/images/family$", local.project)) == 0
  ...
  precondition {
    condition     = var.instance_image_custom || contains(keys(local.known_project_families), local.project)
    error_message = <<-EOD
    Images in project ${local.project} are not published by SchedMD. Images must be created by compatible releases of the Terraform and Packer modules following the guidance at https://goo.gle/hpc-slurm-images. Set var.instance_image_custom to true to silence this error and acknowledge that you are using a compatible image.
    EOD
  }
}
...
output "source_image_family" {
  ...
  precondition {
    condition     = !contains(keys(local.known_project_families), local.project) || try(contains(local.known_project_families[local.project], local.family), false)
    error_message = <<-EOD
    Image family ${local.family} published by SchedMD in project ${local.project} is not compatible with this release of the Terraform Slurm modules. Select from known compatible releases:
    ${join("\n", [for p in try(local.known_project_families[local.project], []) : "\t\"${p}\""])}
    EOD
  }
}
...
# TODO:
# Due to https://github.com/hashicorp/terraform-provider-google/issues/21766 
# we can not perform following check, once issue is resolved restore those checks:
#
# postcondition {
