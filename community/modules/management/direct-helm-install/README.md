...
This module provides a standardized way to deploy Helm charts directly to a GKE cluster. It embeds the Helm provider configuration directly within it, taking a `cluster_id` as input, to fetch the live endpoint and certificate during execution.
...
| [google_container_cluster.target](https://registry.terraform.io/providers/hashicorp/google/latest/docs/data-sources/container_cluster) | data source |
...
| <a name="input_cluster_id"></a> [cluster\_id](#input\_cluster\_id) | The full resource ID of the GKE cluster (e.g., projects/PROJECT\_ID/locations/LOCATION/clusters/CLUSTER\_NAME). Note: PROJECT\_ID, LOCATION, and CLUSTER\_NAME are placeholders. | `string` | n/a | yes |
...
| Name | Description |
| ---- | ----------- |
| <a name="output_release_completion_id"></a> [release\_completion\_id](#output\_release\_completion\_id) | The status of the helm release to act as a dependency anchor |
