...
| <a name="input_service_agent_principals"></a> [service\_agent\_principals](#input\_service\_agent\_principals) | Fully qualified principal strings granted roles/cloudkms.cryptoKeyEncrypterDecrypter on the CryptoKey, e.g. "serviceAccount:service-1234567890@compute-system.iam.gserviceaccount.com". Use this for principals service\_agents cannot derive: agents belonging to a different project, or a user-managed service account. Unioned with service\_agents; each principal must already exist. | `set(string)` | `[]` | no |
