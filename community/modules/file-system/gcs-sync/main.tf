...
locals {
  staged_directories = [
    for d in var.directories : {
      ...
      destination_path = trim(d.destination_path, "/")
      destination_uri  = "gs://${var.bucket_name}/${trim(d.destination_path, "/")}/"
      repo_url         = coalesce(d.repo_url, var.repo_url)
      repo_ref         = coalesce(d.repo_ref, var.repo_ref)
      exclude_regex    = join("|", coalesce(d.exclude, []))
    }
  ]
  ...
  staged_files = [
    for f in var.files : {
      ...
      destination_path = trim(f.destination_path, "/")
      destination_uri = (
        endswith(f.destination_path, "/")
        ? "gs://${var.bucket_name}/${trim(f.destination_path, "/")}/${basename(f.source_path)}"
        : "gs://${var.bucket_name}/${trim(f.destination_path, "/")}"
      )
      repo_url = coalesce(f.repo_url, var.repo_url)
      repo_ref = coalesce(f.repo_ref, var.repo_ref)
    }
  ]
}
