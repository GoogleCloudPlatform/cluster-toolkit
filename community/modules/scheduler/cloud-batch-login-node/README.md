...
- Uses the same VM settings as the first provided `cloud-batch-job`, such as
  image, machine type, etc...
- Runs the same `startup-script` as the first provided `cloud-batch-job`.
- Has the same mounted file systems as the provided `cloud-batch-job`.
- Contains a folder with job templates generated by `cloud-batch-job` modules.
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_login_startup_script"></a> [login\_startup\_script](#module\_login\_startup\_script) | github.com/GoogleCloudPlatform/hpc-toolkit//modules/scripts/startup-script | v1.7.0 |
...
| <a name="input_gcloud_version"></a> [gcloud\_version](#input\_gcloud\_version) | The version of the gcloud cli being used. Used for output instructions.<br>Valid inputs are `\"alpha\"`, `\"beta\"` and \"\" (empty string for default<br>version). Typically supplied by a cloud-batch-job module. If multiple<br>cloud-batch-job modules supply the gcloud\_version, only the first will be used. | `string` | `"alpha"` | no |
| <a name="input_instance_template"></a> [instance\_template](#input\_instance\_template) | Login VM instance template self-link. Typically supplied by a<br>cloud-batch-job module. If multiple cloud-batch-job modules supply the<br>instance\_template, the first will be used. | `string` | n/a | yes |
| <a name="input_job_data"></a> [job\_data](#input\_job\_data) | List of jobs and supporting data for each, typically provided via "use" from the cloud-batch-job module. | <pre>list(object({<br>    template_contents = string,<br>    filename          = string,<br>    id                = string<br>  }))</pre> | n/a | yes |
| <a name="input_job_filename"></a> [job\_filename](#input\_job\_filename) | Deprecated (use `job_data`): The filename of the generated job template file. Typically supplied by a cloud-batch-job module. | `string` | `null` | no |
| <a name="input_job_id"></a> [job\_id](#input\_job\_id) | Deprecated (use `job_data`): The ID for the Google Cloud Batch job. Typically supplied by a cloud-batch-job module for use in the output instructions. | `string` | `null` | no |
| <a name="input_job_template_contents"></a> [job\_template\_contents](#input\_job\_template\_contents) | Deprecated (use `job_data`): The contents of the Google Cloud Batch job template. Typically supplied by a cloud-batch-job module. | `string` | `null` | no |
