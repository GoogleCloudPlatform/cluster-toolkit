...
output "instructions" {
  ...
  Submit your job from login node:
    gcloud ${var.gcloud_version} batch jobs submit ${var.job_id} --location=${var.region} --config=${var.batch_job_directory}/${var.job_filename}
  
  Check status:
    gcloud ${var.gcloud_version} batch jobs describe ${var.job_id} --location=${var.region} | grep state:
  
  Delete job:
    gcloud ${var.gcloud_version} batch jobs delete ${var.job_id} --location=${var.region}
