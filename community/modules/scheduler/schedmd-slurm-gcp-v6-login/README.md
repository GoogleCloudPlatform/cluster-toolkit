...
| <a name="input_instance_metadata"></a> [instance\_metadata](#input\_instance\_metadata) | Metadata applied directly to login instances. Unlike metadata inherited from the instance template, changes to these values update instances in place. | `map(string)` | `{}` | no |
