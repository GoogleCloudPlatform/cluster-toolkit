...
locals {
  output_dir = var.enable_hybrid ? (can(coalesce(var.output_dir)) ? abspath(var.output_dir) : abspath(".")) : null
}
...
tar czf config.tgz --exclude="*.log" --exclude="install_hybrid.sh" --exclude="*.zip" --exclude="config.tgz" . > /dev/null 2>&1
echo "Success"
EOF
