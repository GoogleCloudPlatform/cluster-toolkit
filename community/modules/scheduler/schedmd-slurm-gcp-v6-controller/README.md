...
| <a name="input_controller_state_disk"></a> [controller\_state\_disk](#input\_controller\_state\_disk) |  A persistent disk used to save state of slurm. The disk is not attached to the controllers directly, but instead to a dedicated VM `machine_slurm_state_storage`  deployed via a regional MIG.<br/>  To disable this feature, set this variable to null.<br/><br/>  NOTE: This will not save the contents at /opt/apps and /home. To preserve those, they must be saved externally. | <pre>object({<br/>    type = string<br/>    size = number<br/>  })</pre> | <pre>{<br/>  "size": 50,<br/>  "type": "pd-ssd"<br/>}</pre> | no |
...
| <a name="input_machine_slurm_state_storage"></a> [machine_slurm_state_storage](#input_machine_slurm_state_storage) | Configuration of the VM that hosts the shared Slurm state disk (used in HA deployments). This machine is deployed in a regional MIG for high availability. | <pre>object({<br/>  machine_type = string<br/>  disk = object({<br/>    source_image         = string<br/>    source_image_project = optional(string)<br/>    type                 = string<br/>    size                 = number<br/>  })<br/>})</pre> | <pre>{<br/>  machine_type = "e2-medium",<br/>  disk = {<br/>    source_image = "projects/rocky-linux-cloud/global/images/family/rocky-linux-9",<br/>    type = "pd-balanced",<br/>    size = 50<br/>  }<br/>}</pre> | no |
...
| <a name=" nb_controllers"></a> [nb\_controllers](#input\nb\_controllers) |  Number of Slurm controller nodes to deploy. Use 1 for a single-controller setup or 2 for high availability (HA).| `number` | `2` | no |
...
| <a name="input_slurm_state_ip"></a> [slurm_state_ip](#input_slurm_state_ip) | Optional static internal IP address to assign to the Slurm state storage VM. | `string` | `null` | no |
| <a name="input_slurm_state_storage_scopes"></a> [slurm_state_storage_scopes](#input_slurm_state_storage_scopes) | OAuth scopes to assign to the state storage VM. | `list(string)` | <pre>[ "https://www.googleapis.com/auth/cloud-platform" ]</pre> | no |
...
| <a name="output_slurm_controller_instance"></a> [slurm\_controller\_instance](#output\_slurm\_controller\_instance) | Compute instance of primary controller node |
