...
| <a name="input_slurm_conf_template"></a> [slurm\_conf\_template](#input\_slurm\_conf\_template) | Slurm slurm.conf template. | <pre>object({<br/>    content = optional(string)<br/>    source  = optional(string)<br/>  })</pre> | `{}` | no |
| <a name="input_slurm_conf_tpl"></a> [slurm\_conf\_tpl](#input\_slurm\_conf\_tpl) | DEPRECATED: This variable is deprecated and will be removed in a future version. Please use 'slurm\_conf\_template' object instead. This path is used only if 'slurm\_conf\_template' is not provided. | `string` | `null` | no |
