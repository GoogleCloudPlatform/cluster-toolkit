...
class Lookup:
    ...
    @property
    def spool_mount(self) -> NSMount:
        mnt = NSDict(
            server_ip=self.cfg.slurm_state_ip or self.controller_mount_server_ip(),
            local_mount=slurmdirs.state,
            remote_mount=slurmdirs.state,
            fs_type="nfs",
            mount_options="defaults,hard,intr,_netdev",
        )
        return self.normalize_ns_mount(mnt)
