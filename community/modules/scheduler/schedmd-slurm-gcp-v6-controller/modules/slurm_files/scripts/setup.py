...
def setup_login():
    ...
    lkp = lookup()
    primary_slurmctld_host = f"{lkp.control_host}"
    if lkp.control_addr:
        slurmctld_host = f"{lkp.control_host}({lkp.control_addr})"
    backup_slurmctld_host = lkp.cfg.get("slurm_backup_controller_name")
    if backup_slurmctld_host:
        sackd_options = [
            f'--conf-server="{primary_slurmctld_host}:{lkp.control_host_port}","{backup_slurmctld_host}:{lkp.control_host_port}"',
        ]
    else:
        sackd_options = [
            f'--conf-server="{primary_slurmctld_host}:{lkp.control_host_port}"',
        ]
