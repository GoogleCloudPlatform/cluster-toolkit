...
def get_metadata(path: str, silent: bool = False) -> str:
  ...
  try:
    ...
    if not silent:
      log.warning("metadata not found (%s)", url)
...
def instance_nodeset():
  """Returns the nodeset name of the instance."""
  return instance_metadata("attributes/slurm_nodeset")


def cluster_name():
  """Returns the cluster name of the instance."""
  return instance_metadata("attributes/slurm_cluster_name")


def controller_host():
  """Returns the hostname of the controller."""
...
def install_custom_scripts():
  ...
  for blob in blob_list():
    ...
    if not m:
      log.warning("blob '%s' that doesn't match expected pattern", blob.name)
      continue
    ...
    need_update = True
    if fullpath.exists():
      need_update = file_md5_hash(fullpath) != blob.md5_hash

    if need_update:
      log.info("installing custom script: %s from %s", path, blob.name)
      with fullpath.open("wb") as f:
        blob.download_to_file(f)
      chown_slurm(fullpath, mode=0o755)


def file_md5_hash(fullpath: Path) -> str:
...
class NSMount:
  ...
  @classmethod
  def from_dict(cls, data: dict[str, str]) -> "NSMount":
    return NSMount(
...
class Config:
  ...
  def from_dict(cls, data: dict[str, Any]) -> "Config":
    return Config(
        network_storage=[
            NSMount.from_dict(ns) for ns in data.get("network_storage", [])
        ],
...
def blob_list():
  """Returns a list of blobs in the bucket with the given prefix."""
  bucket_name, prefix = _get_bucket_and_common_prefix()
  return list(storage_client().list_blobs(bucket_name, prefix=f"{prefix}/"))


def fetch_config() -> bool:
  """Fetches the config file from GCS if it has changed."""
