...
def update_topology_yaml(lkp: util.Lookup):
    log.info("Running slurmsync instance synchronization.")
    lkp = util.lookup()
    ...
    if existing_combined_nodelist_str != expected_combined_nodelist_str:
        try:
            ...
            util.run("scontrol reconfig", timeout=30)
        except Exception as e:
            log.error(f"Failed to regenerate topology.yaml: {e}", exc_info=True)
...
def main():
    ...
    if lkp.is_controller:
        ...
        try:
            update_topology_yaml(lkp)
        except Exception:
            log.exception("failed to sync topology.yaml")
