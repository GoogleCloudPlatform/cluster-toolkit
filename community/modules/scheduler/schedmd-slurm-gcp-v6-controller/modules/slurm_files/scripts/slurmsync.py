...
def get_mig_from_node(nodename: str):
    accelerator_topo = util.lookup().node_nodeset(nodename)["accelerator_topology"]
...
def sync_migs():
    ...
    for mig_name, mig_obj in all_migs.items():
        ...
        if len(mig_nodes) == 0:
            ...
            age = util.now() - creationTimestamp
            if age > threshold:
        ...
        if (mig_obj.versions[0] != node_nodeset.instance_template) & len(mig_nodes) == 0:
            migs_to_delete.append(mig_obj)
            continue
