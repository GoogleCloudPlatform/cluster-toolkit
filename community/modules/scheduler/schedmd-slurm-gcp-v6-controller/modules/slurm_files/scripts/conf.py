...
def conflines(lkp: util.Lookup) -> str:
    ...
    any_dynamic = any(p.has_dynamic for p in lkp.partitions)
...
def partition_lines(partition: Partition, lkp: util.Lookup) -> str:
    ...
    power_down_on_idle = partition.enable_job_exclusive and not partition.has_dynamic
...
def suspend_exc_lines(lkp: util.Lookup) -> Iterable[str]:
    ...
    dyn_parts = [p.name for p in lkp.partitions if p.has_dynamic]
...
def make_cloud_conf(lkp: util.Lookup) -> str:
    ...
    lines = [
        ...
        *(partition_lines(p, lkp) for p in lkp.partitions),
