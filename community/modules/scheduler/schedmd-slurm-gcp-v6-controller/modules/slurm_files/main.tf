...
locals {
  ...
  controller_script_files = {
    ...
    : format("slurm-controller-script-%s", replace(basename(x.filename), local.ok_chars, "_")) => x.content
  }
  ...
  nodeset_script_files = { for x in flatten([
    ...
    : [for s in scripts
      : {
        content = s.content,
      name = format("slurm-nodeset-%s-script-%s", nodeset, replace(basename(s.filename), local.ok_chars, "_")) }
  ]]) : x.name => x.content }
  ...
  login_script_files = { for x in flatten([
    ...
    : [for s in scripts
      : {
        content = s.content,
      name = format("slurm-login-%s-script-%s", group, replace(basename(s.filename), local.ok_chars, "_")) }
  ]]) : x.name => x.content }
}
