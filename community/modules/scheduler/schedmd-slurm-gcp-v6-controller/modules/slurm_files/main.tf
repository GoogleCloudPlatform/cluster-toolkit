...
locals {
  ...
  hybrid_conf = {
    ...
    slurm_version           = try(var.hybrid_conf.slurm_version, null)
  }

  munge_mount = (var.enable_hybrid || var.slurm_backup_controller_name != null) ? {
    server_ip     = lookup(var.munge_mount, "server_ip", var.enable_hybrid ? coalesce(local.hybrid_conf.slurm_control_addr, local.hybrid_conf.slurm_control_host) : coalesce(var.slurm_control_addr, var.slurm_control_host, "$controller"))
