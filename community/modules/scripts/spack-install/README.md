...
| <a name="input_environments"></a> [environments](#input\_environments) | Defines spack environments to configure, given as a list.<br>  Each environment must define a name, and type.<br>  Valid types for environments are 'file', and 'packages'.<br>  Environments of type 'packages' must define a list named packages, containing<br>  all of the packages to install in that environment.<br>  Environments of type 'file' must define a string named 'content' containing the<br>  contents of a spack.yaml file to create the environment from. | `list(map(any))` | `[]` | no |
