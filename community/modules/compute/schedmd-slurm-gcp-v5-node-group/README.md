...
| <a name="input_reservation_name"></a> [reservation\_name](#input\_reservation\_name) | Name of the reservation to use for VM resources; set to empty string if no reservation is used. | `string` | `""` | no |
| <a name="input_scheduling"></a> [scheduling](#input\_scheduling) | Specifies the scheduling options for the instances.<br>  See: https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_instance#nested_scheduling | `map(string)` | `{}` | no |
