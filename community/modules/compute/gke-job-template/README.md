...
| <a name="input_gpu_per_node"></a> [gpu\_per\_node](#input\_gpu\_per\_node) | The gpus per node. Generally populated from gke-node-pool via `use` field. The lowest value provided will be used. | `list(number)` | <pre>[<br>  0<br>]</pre> | no |
