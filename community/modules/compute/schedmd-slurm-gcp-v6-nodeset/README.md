...
| <a name="input_skip_reservation_validation"></a> [skip\_reservation\_validation](#input\_skip\_reservation\_validation) | If true, skips validation of the reservation. This is helpful if the user or service account does not have read access to a reservation but has access to use the reservation. | `bool` | `false` | no |
