...
| <a name="input_provisioning_engine"></a> [provisioning\_engine](#input\_provisioning\_engine) | Compute node provisioning engine: 'AUTO', 'MIG', or 'BULK\_INSERT'. Note: When using 'MIG', 'node\_count\_dynamic\_max' must be explicitly set to 0 and 'enable\_placement' must be set to false. | `string` | `"AUTO"` | no |
