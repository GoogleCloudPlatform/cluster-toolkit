...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_accelerator_topology"></a> [accelerator\_topology](#input\_accelerator\_topology) | Specifies the shape of the Accelerator (GPU/TPU) slice. Note: When set, 'enable\_placement' must be set to true and 'node\_count\_dynamic\_max' must be explicitly set to 0. | `string` | `null` | no |
