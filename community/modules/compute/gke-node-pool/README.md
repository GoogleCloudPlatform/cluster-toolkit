...
| <a name="input_local_ssd_count"></a> [local\_ssd\_count](#input\_local\_ssd\_count) | The number of local SSDs to attach to each node to back ephemeral storage.<br>Uses NVMe interfaces.  Must be supported by `machine_type` - see <br>https://cloud.google.com/compute/docs/disks#local_ssd_machine_type_restrictions<br>Note that depending on the intended use, additional steps may be needed to <br>allow container access to these disks.  See <br>https://cloud.google.com/kubernetes-engine/docs/how-to/persistent-volumes/local-ssd<br>for more information. | `number` | `0` | no |
