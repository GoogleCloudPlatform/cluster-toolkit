...
| <a name="input_repository_kms_key_name"></a> [repository\_kms\_key\_name](#input\_repository\_kms\_key\_name) | The full resource name of a Cloud KMS CryptoKey used to encrypt the<br/>Artifact Registry repository contents (CMEK). The Artifact Registry<br/>service agent must hold roles/cloudkms.cryptoKeyEncrypterDecrypter on<br/>the key before repository creation, and the key location must match<br/>the repository location. Encryption mode is fixed at creation.<br/>This is distinct from user\_managed\_replication[].kms\_key\_name, which<br/>encrypts only the Secret Manager secret created for remote-repository<br/>credentials. | `string` | `null` | no |
