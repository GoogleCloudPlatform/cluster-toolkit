...
def write_HPL_input(N, NB, PQ_Grids, outputfile='hpccinf.txt'):
...
def parse_hpl_out(outFile):
    with outFile.open('r') as fp:
        for line in fp:
            ...
            if len(v) == 2:
                if v[0] == 'HPL_Tflops':
                    return {'result_unit': 'Tflops', 'result_value': v[1]}
    return None
...
if __name__ == '__main__':
    ...
    # Now, run HPL itself
    subprocess.run(["mpirun", "hpcc"])

    outFile = Path('./hpccoutf.txt')
    if outFile.exists():
        # Create KPI.json
