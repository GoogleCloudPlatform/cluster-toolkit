...
OLD_TAG=$(gh release list -R rahimkhan19/cluster-toolkit -L 1 --json tagName --jq '.[] | .tagName')
...
PR_URL=$(gh pr create --repo rahimkhan19/cluster-toolkit --base "${RC_BRANCH}" --head "${V_BRANCH}" \
	--title "Update Toolkit release to ${NEW_TAG}" \
	--body "Set release-candidate to version ${NEW_VERSION}" || gh pr list --repo rahimkhan19/cluster-toolkit --state open --head "${V_BRANCH}" --base "${RC_BRANCH}" --json url --jq '.[0].url')
echo "$PR_URL"
echo
echo
echo
