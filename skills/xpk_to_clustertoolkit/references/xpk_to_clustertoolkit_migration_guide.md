...
**Target Audience**: Google Cloud customers currently using the `xpk` CLI tool for GKE TPU/GPU cluster deployment and AI/ML workload orchestration.
...
- [4. Step 2: Cluster Infrastructure Migration (`xpk cluster create`)](#4-step-2-cluster-infrastructure-migration-xpk-cluster-create)
- [5. Step 3: Workload Migration (`xpk workload create`)](#5-step-3-workload-migration-xpk-workload-create)
- [6. Step 4: Advanced Workloads & Features](#6-step-4-advanced-workloads--features)
- [7. Complete Command Quick-Reference Table](#7-complete-command-quick-reference-table)

---

## 1. Executive Summary & Overview

Google Cloud is standardizing AI/ML infrastructure orchestration on **Cluster Toolkit (`gcluster`)**. Cluster Toolkit provides a unified framework for both **infrastructure provisioning** (via modular, declarative Terraform blueprints) and **workload submission** (via `gcluster job submit` integrated with JobSet and Kueue).
...
- **Inline Storage Mounting & Mount Options**: Mount Cloud Storage buckets (`gs://`), PVCs (`pvc://`), or Filestore (`filestore://`) inline using the `--mount` flag with custom GCS Fuse options (`options=<options>`), replacing separate `xpk storage attach` workflows.
...
| **Mount Options** | `--storage-options` | Inline `options=<options>` within `--mount` (GCS `gs://` volumes exclusively) |
...
```bash
# Download latest gcluster binary (Linux amd64)
curl -L -O https://github.com/GoogleCloudPlatform/cluster-toolkit/releases/latest/download/gcluster-linux-amd64.tar.gz
tar -xzf gcluster-linux-amd64.tar.gz
sudo mv gcluster /usr/local/bin/gcluster

# Verify installation
gcluster --version
```

### GCP Authentication & Environment Configuration

Authenticate `gcloud` and configure your GCP project and Artifact Registry repository for on-the-fly image builds:
...
## 4. Step 2: Cluster Infrastructure Migration (`xpk cluster create`)
...
2. **Deploy**: Run `gcluster create` and `gcluster deploy`.
...
```bash
# 1. Expand blueprint into Terraform modules and deployment folder
gcluster create my-tpu-cluster.yaml

# 2. Provision GKE cluster and TPU node pools on GCP
gcluster deploy my-tpu-cluster
```

> [!TIP]
> **Utilizing Production-Ready Example Blueprints**: Cluster Toolkit includes a comprehensive library of validated blueprints within the [`examples/`](https://github.com/GoogleCloudPlatform/cluster-toolkit/blob/main/examples) repository for both TPU (v4 through v7x) and GPU (A3/A4) architectures. These templates allow for immediate deployment by injecting runtime overrides through the `--vars` flag, eliminating the necessity of crafting a custom YAML definition from scratch.
>
> ```bash
> gcluster create examples/gke-tpu-v6e/gke-tpu-v6e.yaml \
>   --vars project_id=my-gcp-project,zone=us-central1-a,deployment_name=my-tpu-cluster,num_slices=2
> ```

#### Deployment Workspace & Generated Artifacts

Note that running `gcluster deploy` transparently executes `gcluster create` automatically if the deployment directory does not exist. You only need to run `gcluster create` separately if you wish to inspect or customize the generated code prior to deployment.

When `gcluster create` executes, it expands your declarative blueprint into a dedicated deployment workspace (e.g., `my-tpu-cluster/`). Within this workspace, `gcluster` generates the following visible files and directories:
...
- **`.ghpc/`**: An internal workspace metadata directory containing state tracking and the fully expanded blueprint definition (`.ghpc/artifacts/expanded_blueprint.yaml`).

#### Externalizing Configuration via Deployment Files (`-d`)

To maintain a clean separation between architectural logic and environment-specific data, you can ingest variables from a standalone file using the `-d` parameter:

```bash
gcluster create -d my-cluster-deployment.yaml my-tpu-cluster.yaml
```

Consult the [TPU v6e Blueprint Reference](https://github.com/GoogleCloudPlatform/cluster-toolkit/blob/main/examples/gke-tpu-v6e/README.md#create-a-cluster-using-cluster-toolkit) for detailed guidance on implementing deployment variable files.

---

## 5. Step 3: Workload Migration (`xpk workload create`)
...
| `--project <ID>` | `--project <ID>` | Target GCP project (optional if set in `gcloud config`) |
...
| `--storage <NAME>` | `--mount <SRC>;<DEST>[;<MODE>][;options=<OPTS>]` | Inline storage mount (`gs://`, `pvc://`, `filestore://`). Mode defaults to `rw`. **Note**: `options=` is supported exclusively for GCS volumes (`gs://`) |
...
### A. Parallel Containers on TPU v7x
...
## 7. Complete Command Quick-Reference Table

| XPK Command | Cluster Toolkit (`gcluster`) Equivalent | Notes |
| :--- | :--- | :--- |
| `xpk cluster create` | `gcluster create <blueprint_file.yaml> && gcluster deploy <deployment_name>` | Provision GKE cluster & TPU node pools |
...
| `xpk config set` | `gcluster job config set` | Update local CLI config |
