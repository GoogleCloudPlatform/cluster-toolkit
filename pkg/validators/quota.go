...
func verifyQuotas(client QuotaClient, reqs []QuotaRequirement) error {
	...
	for _, req := range reqs {
		...
		if req.Region == "global" {
			...
			if err != nil {
				_ = errs.Add(fmt.Errorf("failed to get project %s: %w", projID, err))
				continue
			}
			...
			if err != nil {
				_ = errs.Add(fmt.Errorf("failed to get region %s: %w", req.Region, err))
				continue
			}
		...
		if usage+req.Needed > limit {
			_ = errs.Add(fmt.Errorf("insufficient quota for %s in %s (project: %s). Requested: %.2f, Used: %.2f, Limit: %.2f",
