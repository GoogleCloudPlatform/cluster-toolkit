...
- [7. XPK <> CT Command Mappings Table](#7-xpk--ct-command-mappings-table)
...
> [!NOTE]
> **Artifact Registry configuration for on-the-fly builds (`GCLUSTER_IMAGE_REPO`):**
>
> - When you use `--base-image` with `--build-context`, `gcluster` automatically constructs the target image registry path using your cluster's region and project ID: `REGION-docker.pkg.dev/PROJECT_ID/IMAGE_REPO/IMAGE_NAME:TAG` (where `IMAGE_REPO` is the value of `$GCLUSTER_IMAGE_REPO`).
> - **Cross-region / cross-project constraint:** The on-the-fly container image builder currently does not support Artifact Registry repositories located in a different region or Google Cloud project than the target cluster. If your workflow relies on a centralized registry across projects or regions, build and push your container image independently, and supply the full image URI directly via `--image`.
...
> [!IMPORTANT]
> **In-place cluster migration is not supported:** In-place migration of an existing `xpk`-provisioned cluster to Cluster Toolkit is not supported. To migrate your cluster infrastructure, you must provision a new cluster with `gcluster deploy` and decommission the existing `xpk` cluster with `xpk cluster delete`. This guide focuses on translating your infrastructure blueprints, automation scripts, and workload submission workflows.
...
> [!WARNING]
> **Important Constraint on `options=`**: The `options=` parameter is strictly validated by `gcluster` and is supported exclusively for Cloud Storage volumes (`gs://`). Appending options to non-GCS volumes (such as `filestore://` or PVCs) will result in a CLI validation error.
...
## 7. XPK <> CT Command Mappings Table
