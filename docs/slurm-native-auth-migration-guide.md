...
- **CI/CD Integration Tests**: Automated testing playbooks (e.g., `slurm-integration-test.yml`) have been updated to expect Slurm Native Authentication. If you run tests against older MUNGE clusters, test pipelines may time out waiting for `/etc/slurm/slurm.key` instead of `/var/run/munge/munge.socket.2`.
