...
- **External Submission Nodes**: Custom submission nodes residing on-premises or in hybrid networks must have the `/etc/slurm/slurm.key` securely distributed to them manually by administrators. Detailed instructions can be found in the [Hybrid Slurm Cluster Guide](hybrid-slurm-cluster/on-prem-instructions.md#distributing-slurm-native-authentication-token-manually). Ensure file ownership is mapped precisely to the native Slurm user account with `0400` permissions.
