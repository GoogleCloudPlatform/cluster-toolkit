...
* **Enable dynamic slicing flag:** Set `enable_dynamic_slicing_for_tpus: true` in the `vars` block. This setting deploys the GKE TPU Slice Controller, automatically sets `accelerator_topology_mode` to `"PROVISION_ONLY"` on the `workload_policy` resource policy, and configures dynamic partition-level topology definitions.
