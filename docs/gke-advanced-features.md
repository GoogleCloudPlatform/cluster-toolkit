...
* **Enable dynamic slicing flag:** Set `enable_dynamic_slicing_for_tpus: true` in the `vars` block. This setting deploys the GKE TPU Slice Controller, sets `accelerator_topology_mode: "PROVISION_ONLY"` on the workload policy, and configures dynamic partition-level topology definitions.
